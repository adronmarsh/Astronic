...
@section('content')
    <form method="POST" action="{{ route('posts.store') }}" enctype="multipart/form-data" class="col-md-6 mx-auto">
        @csrf
        <h1 class="mt-5">{{ __('messages.upload-text') }}</h1>
        <div class="form-group mt-5">
            <label for="title">{{ __('messages.upload-name') }}</label>
        ...
        <div class="form-group mt-5">
            <label for="content">{{ __('messages.upload-content') }}</label>
        ...
        <div class="form-group mt-5">
            <label for="media" class="btn btn-transparent">
                <i class="fas fa-upload"></i>{{ __('messages.upload-file') }}
            </label>
        ...
        <button type="submit" class="btn btn-primary mt-5">{{ __('messages.upload-create') }}</button>
    </form>
@endsection
